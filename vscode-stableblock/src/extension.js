...
function gSide(a,b,g){var fx=a.x*g+a.w*g/2,fy=a.y*g+a.h*g/2,tx=b.x*g+b.w*g/2,ty=b.y*g+b.h*g/2,dx=tx-fx,dy=ty-fy;if(Math.abs(dy)>Math.abs(dx))return dy>0?{fs:'bottom',ts:'top'}:{fs:'top',ts:'bottom'};return dx>0?{fs:'right',ts:'left'}:{fs:'left',ts:'right'};}
...
function sPt(p,side,d){if(side==='top')return{x:p.x,y:p.y-d};if(side==='bottom')return{x:p.x,y:p.y+d};if(side==='left')return{x:p.x-d,y:p.y};return{x:p.x+d,y:p.y};}
function bP(f,t,fs,ts){var stub=12,f2=sPt(f,fs,stub),t2=sPt(t,ts,stub),mx=(f2.x+t2.x)/2,my=(f2.y+t2.y)/2;var bez=Math.abs(t2.y-f2.y)>Math.abs(t2.x-f2.x)?"C"+f2.x+","+my+" "+t2.x+","+my+" "+t2.x+","+t2.y:"C"+mx+","+f2.y+" "+mx+","+t2.y+" "+t2.x+","+t2.y;return"M"+f.x+","+f.y+" L"+f2.x+","+f2.y+" "+bez+" L"+t.x+","+t.y;}
