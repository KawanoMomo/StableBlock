...
function pPos(b,side,idx,total,g){var bx=b.x*g,by=b.y*g,bw=b.w*g,bh=b.h*g,pad=total<=2?0.4:total<=4?0.25:0.15,t=total===1?0.5:pad+(1-2*pad)*idx/(total-1);if(side==='top')return{x:bx+bw*t,y:by};if(side==='bottom')return{x:bx+bw*t,y:by+bh};if(side==='left')return{x:bx,y:by+bh*t};return{x:bx+bw,y:by+bh*t};}
...
function bP(f,t,fs,ts){var dx=t.x-f.x,dy=t.y-f.y,dist=Math.sqrt(dx*dx+dy*dy),cpd=Math.max(30,dist*0.4),isH=(fs==='right'||fs==='left'),gap=isH?Math.abs(dx):Math.abs(dy);if(gap>0)cpd=Math.min(cpd,gap*0.45);var c1=eP(f,fs,cpd),c2=eP(t,ts,cpd);return"M"+f.x+","+f.y+" C"+c1.x+","+c1.y+" "+c2.x+","+c2.y+" "+t.x+","+t.y;}
